...
public class ChainLetter {
	...
	public static void main(String[] args) throws Exception {
		...
		DataSet<Tuple2<Long, Long>> initialForwards = selectedInitiators.join(edges).where(0).equalTo(0)
		...
		DataSet<Tuple1<Long>> deliverMessage = deltaIteration.getWorkset().project(1).types(Long.class)
				.distinct();

		DataSet<Tuple2<Long, Boolean>> nextRecipientStates = deltaIteration.getSolutionSet()
				.join(deliverMessage).where(0).equalTo(0).flatMap(new ReceiveMessage());

		DataSet<Tuple2<Long, Long>> nextForwards = nextRecipientStates.join(edges).where(0).equalTo(0)
				.flatMap(new ForwardToFriend());

		DataSet<Tuple2<Long, Boolean>> result = deltaIteration.closeWith(nextRecipientStates, nextForwards);
	...
	public static class InitialForwards implements
			FlatMapFunction<Tuple2<Tuple2<Long, Boolean>, Tuple2<Long, Long>>, Tuple2<Long, Long>> {

		private final Random random = new Random(Config.randomSeed());

		@Override
		public void flatMap(Tuple2<Tuple2<Long, Boolean>, Tuple2<Long, Long>> vertexWithEdge,
	...
	public static class ReceiveMessage implements
		...
		@Override
		public void flatMap(Tuple2<Tuple2<Long, Boolean>, Tuple1<Long>> recipients,
	...
	public static class ForwardToFriend implements
			FlatMapFunction<Tuple2<Tuple2<Long, Boolean>, Tuple2<Long, Long>>, Tuple2<Long, Long>> {
